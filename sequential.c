...
double calculate_normal_distribution(double x, double mean, double stddev);
...
int main() {
    ...
    // Print table format without row limit
    printf("\n%-10s %-10s %-10s %-15s %-10s %-10s %-10s %-10s\n", 
           "Row", "Mean", "Median", "Variance", "Stddev", "Range", "Pearson", "NormalDist");

    for (int i = 0; i < data_count; i++) {
        double normal_dist_confirmed = calculate_normal_distribution(confirmed[i], mean_confirmed, stddev_confirmed);
        printf("%-10d %-10.2f %-10.2f %-10.2f %-10.2f %-10.2f %-10.2f %-10.5f\n", 
               i + 1, confirmed[i], median_confirmed, variance_confirmed, stddev_confirmed, 
               range_confirmed, pearson_confirmed_deaths, normal_dist_confirmed);
    }
...
double calculate_normal_distribution(double x, double mean, double stddev) {
    return (1 / (stddev * sqrt(2 * 3.14))) * exp(-0.5 * pow((x - mean) / stddev, 2));
}